...
import McKuroApi, { CARD_POOL_TYPE, type TGaChaResData, type TResourceType } from '../api';
import util from '../util';
import config from '../config';

export type TCardPoolType = 'UP' | '常驻';

export type TGachaLink = {
  user_id: string;
  url: string;
};

export type TGaChaJson = object;

export type TGaChaData = {
...
export default class GaChaModel {
  ...
  getGachaData(type: TCardPoolType = 'UP') {
    ...
    const roleGachaData: TGaChaResData[] = this.readGachaJSON(`${roleGachaType}`) || [];
    const weaponGachaData: TGaChaResData[] = this.readGachaJSON(`${weaponGachaType}`) || [];
   ...
   */
  countGacha(listData: TGaChaResData[], typeId: CARD_POOL_TYPE) {
    let numOf4Star = 0;
    let numOf5star = 0;
    let lastNum = 0;
   ...
   */
  updateGachaJson(newData: TGaChaResData[], type: CARD_POOL_TYPE) {
    // 初始化更新数量计数器
    let updateNum = 0;
    // 根据扭蛋资源类型生成文件名
    const fileName = `${type}`;
    // 读取现有的扭蛋资源数据数组，如果文件不存在则返回空数组
    const oldData: TGaChaResData[] = this.readGachaJSON(fileName) || [];
  ...
  writeGachaJSON(fileName: string, data: TGaChaResData[]) {
    return util.writeJSON(`${this.#jsonUserGachaPath}/${fileName}.json`, data);
  }

  getCountTime(gachaData: TGaChaResData[]) {
