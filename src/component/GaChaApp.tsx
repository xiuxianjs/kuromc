...
import { TCardPoolType, TGaChaData, TGaCheViewData } from '../models/gacha';
    ...
    <div>
      <div className='bg-black bg-opacity-50 rounded-lg overflow-hidden w-[96px] h-[130px]'>
        <div className='p-4 relative'>
          ...
          <div className={`absolute left-0 bottom-0 h-[50%] w-[100%] kuromc-level-${item.星级 || record.src.qualityLevel}-bgc`} />
          {item.up && (
        ...
        <div className='py-1 text-center'>
          <span className={`text-[20px] font-bold ${conutTextColorClassName}`}>{count}</span> 抽
        </div>
      </div>
    </div>
  );
...
export type GaChaAppProps = {
  roleData: TGaCheViewData;
  weaponData: TGaCheViewData;
  type: TCardPoolType;
};

export default function GaChaApp({ roleData, weaponData, type }: GaChaAppProps) {
        ...
        <div>
          <div className='relative'>
            <div className='kuromc-text-title text-4xl font-bold'>
              <span>抽卡分析</span>
              <img className='mt-[4px] h-[36px] w-[210px]' src='https://web-static.kurobbs.com/resource/wiki/prod/assets/title-underline-mc-3c8ec006.svg' />
            </div>
            <div className='kuromc-gacha-bottom-line'>
              <div className='black-block' />
              <div className='black-block' style={{ right: -1, left: 'auto' }} />
            </div>
          </div>
          ...
          <div className='kuromc-gacha-body'>
            <div className='mt-10 border-l-4 pl-4 kuromc-text border-[#f2d57c]'>
              ...
              {roleData.countStartTime && roleData.countEndTime && (
                <div className='kuromc-text mt-10 text-center opacity-80'>
                  数据统计时间: {roleData.countStartTime} ~ {roleData.countEndTime}
                </div>
              )}
            </div>
            ...
            <div className='mt-10 border-l-4 pl-4 kuromc-text border-[#f2d57c]'>
              <div className='py-4 flex justify-between items-center'>
                ...
                <div>
                  剩余
                  <span className='mx-2 text-xl'>{80 - weaponData.lastNum}</span>
                  次唤取出五星
                </div>
              </div>
              <div className='grid grid-cols-7 gap-4'>
              ...
              {weaponData.countStartTime && weaponData.countEndTime && (
                <div className='kuromc-text mt-10 text-center opacity-80'>
                  数据统计时间: {weaponData.countStartTime} ~ {weaponData.countEndTime}
                </div>
              )}
            </div>
          </div>
