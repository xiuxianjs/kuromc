...
import { apiWikiCoreCatalogueItemGetPage } from './mc.js';
...
class Data {
    ...
    getAympathizer = async () => {
        ...
        return status === 'local'
            ? this.#localData('aympathizer.json', await apiWikiCoreCatalogueItemGetPage())
    ...
    getWeapon = async () => {
        ...
        const status = this.connect();
        return status === 'local'
            ? this.#localData('weapon.json', await apiWikiCoreCatalogueItemGetPage({ catalogueId: '1106' }))
            : this.#redisData(key, await apiWikiCoreCatalogueItemGetPage({ catalogueId: '1106' }));
    };
    /**
    ...
    getWeaponProjection = async () => {
        ...
        const status = this.connect();
        return status === 'local'
            ? this.#localData('weaponProjection.json', await apiWikiCoreCatalogueItemGetPage({ catalogueId: '1315' }))
            : this.#redisData(key, await apiWikiCoreCatalogueItemGetPage({ catalogueId: '1315' }));
    };
}
const DB = new Data();
